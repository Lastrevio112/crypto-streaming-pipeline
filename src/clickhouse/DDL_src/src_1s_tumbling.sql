CREATE TABLE IF NOT EXISTS kafka_derived_ohlcv_1s_tumbling
(
...
SETTINGS
    ...
    kafka_topic_list                = 'derived_ohlcv_1s_tumbling',
    kafka_group_name                = 'clickhouse_ohlcv_1s_tumbling_v3',
...
CREATE TABLE IF NOT EXISTS src_1s_tumbling
(
    coin_symbol                 LowCardinality(String),
    window_start                DateTime64(3) CODEC(Delta(8), ZSTD(2)),
    window_end                  DateTime64(3) CODEC(Delta(8), ZSTD(2)),
...
CREATE MATERIALIZED VIEW mv_src_1s_tumbling
TO src_1s_tumbling
AS
SELECT
    coin_symbol,
    ...
    sum_price,
    sum_price_sq
FROM kafka_derived_ohlcv_1s_tumbling
